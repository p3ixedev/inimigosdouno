...
const ACOES = ['bloqueio', 'reverso', '+2'];
const ESPECIAIS = ['+4', 'coringa'];
...
function podePagar(carta, topo, corAtual, acumulado) {
...
function validarSequencia(cartas, topo, corAtual, acumulado) {
...
function calcularEfeito(cartas) {
...
function calcularEfeitoComJogadores(cartas, totalJogadores) {
...
function proximoJogador(jogadores, atual, ordem, pular = false) {
...
function comprarCartas(n, baralho, pilha) {
  let b = [...baralho];
  let p = [...pilha];
  const compradas = [];
  for (let i = 0; i < n; i++) {
...
function criarEstadoInicial(jogadores) {
...
function processarJogada(estado, jogadorId, cartasJogadas, corEscolhida = null) {
...
function escolherCor(estado, jogadorId, cor) {
  if (estado.turnoAtual !== jogadorId) return { erro: 'Não é sua vez!' };
...
function comprarCarta(estado, jogadorId) {
  if (estado.turnoAtual !== jogadorId) return { erro: 'Não é sua vez!' };
...
function acaoZero(estado, jogadorId, acao, alvoId) {
...
function declararUno(estado, jogadorId) {
  return { ...estado, unoDeclarado: { ...estado.unoDeclarado, [jogadorId]: true } };
}

module.exports = {
  CORES, NUMEROS, ACOES, ESPECIAIS,
  criarBaralho, embaralhar, distribuirCartas,
  podePagar, validarSequencia, calcularEfeito,
  calcularEfeitoComJogadores, proximoJogador,
  comprarCartas, criarEstadoInicial,
  processarJogada, escolherCor, comprarCarta,
  acaoZero, declararUno,
};