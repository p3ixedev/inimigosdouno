...
# ===== Inimigos do Uno - Matches API =====
@api_router.get("/matches", response_model=List[Match])
async def list_matches():
    docs = await db.matches.find({}, {"_id": 0}).sort("ts", -1).to_list(2000)
    return docs

@api_router.post("/matches", response_model=Match)
async def create_match(payload: MatchCreate):
    if len(payload.played) < 2:
        from fastapi import HTTPException
        raise HTTPException(status_code=400, detail="At least 2 players required")
    if len(payload.winners) == 0:
        from fastapi import HTTPException
        raise HTTPException(status_code=400, detail="At least 1 winner required")
    for w in payload.winners:
        if w not in payload.played:
            from fastapi import HTTPException
            raise HTTPException(status_code=400, detail="Winners must be in played list")
