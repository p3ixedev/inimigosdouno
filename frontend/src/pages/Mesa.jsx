...
import { Crown, Home, RotateCcw, ChevronRight, Copy, Check, Trophy } from 'lucide-react';
...
const COR = {
  vermelho: { bg: 'bg-[oklch(0.63_0.24_27)]', from: 'oklch(0.68 0.23 27)', to: 'oklch(0.52 0.22 27)', glow: 'shadow-[0_0_30px_oklch(0.63_0.24_27/0.7)]', btn: 'bg-[oklch(0.63_0.24_27)] hover:bg-[oklch(0.68_0.24_27)]', hex: 'oklch(0.63 0.24 27)' },
  azul:     { bg: 'bg-[oklch(0.6_0.22_255)]',  from: 'oklch(0.65 0.21 255)', to: 'oklch(0.48 0.21 255)', glow: 'shadow-[0_0_30px_oklch(0.6_0.22_255/0.7)]',  btn: 'bg-[oklch(0.6_0.22_255)] hover:bg-[oklch(0.65_0.22_255)]',  hex: 'oklch(0.6 0.22 255)' },
  verde:    { bg: 'bg-[oklch(0.68_0.2_152)]',  from: 'oklch(0.72 0.19 152)', to: 'oklch(0.55 0.19 152)', glow: 'shadow-[0_0_30px_oklch(0.68_0.2_152/0.7)]',  btn: 'bg-[oklch(0.68_0.2_152)] hover:bg-[oklch(0.73_0.2_152)]',  hex: 'oklch(0.68 0.2 152)' },
  amarelo:  { bg: 'bg-[oklch(0.85_0.18_90)]',  from: 'oklch(0.88 0.17 90)', to: 'oklch(0.72 0.18 90)', glow: 'shadow-[0_0_30px_oklch(0.85_0.18_90/0.7)]',  btn: 'bg-[oklch(0.85_0.18_90)] hover:bg-[oklch(0.88_0.18_90)]',  hex: 'oklch(0.85 0.18 90)' },
};
...
// Confete gerado uma unica vez (constante do modulo, nao recalcula a cada render)
const CONFETTI_PIECES = Array.from({ length: 20 }).map((_, i) => {
  const angle = (i / 20) * Math.PI * 2;
  const dist = 85 + (i % 3) * 34;
  const cores = [COR.vermelho.hex, COR.azul.hex, COR.verde.hex, COR.amarelo.hex];
  return {
    id: i,
...
// Carta na mao do jogador - possui layoutId para poder "voar" ate a pilha quando jogada
function CartaMao({ carta, selecionada, onClick, disabled }) {
  ...
  const gradFrom = corObj ? corObj.from : '#3f3f46';
  const gradTo = corObj ? corObj.to : '#18181b';
  ...
  return (
    <motion.button
      ...
      animate={selecionada ? { y: -20, scale: 1.12, opacity: 1 } : { y: 0, scale: 1, opacity: 1 }}
      onClick={onClick}
      disabled={disabled}
      className={`card-face relative flex-shrink-0 select-none rounded-xl w-11 h-16 sm:w-14 sm:h-20 ${selecionada ? 'ring-4 ring-white shadow-[0_0_25px_rgba(255,255,255,0.5)]' : 'ring-1 ring-white/25'} ${disabled ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer'}`}
      style={{
        background: `linear-gradient(150deg, ${gradFrom} 0%, ${gradTo} 100%)`,
        boxShadow: selecionada ? '0 10px 28px rgba(0,0,0,0.5)' : '0 4px 10px rgba(0,0,0,0.35)',
      }}
    >
      <div className="absolute inset-[12%] rounded-[50%] opacity-30" style={{ background: corEscuro, transform: 'rotate(-20deg)', borderRadius: '50% / 60%' }} />
      <div className="absolute inset-x-0 top-0 h-1/2 rounded-t-xl bg-white/10 pointer-events-none" />
      {isDestaque && <span className="absolute right-1 top-1 h-1.5 w-1.5 rounded-full bg-white/70" />}
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center p-2">
        ...
        {sub && <span className="mt-0.5 text-white/60 font-bold" style={{ fontSize: '5px', letterSpacing: '0.08em' }}>{sub}</span>}
      </div>
...
// Carta no topo da pilha - compartilha layoutId com a carta de origem na mao, criando a transicao de "voo"
function CartaTopo({ carta, corAtual }) {
  ...
  const gradFrom = corObj ? corObj.from : '#52525b';
  const gradTo = corObj ? corObj.to : '#27272a';
  ...
  return (
    <motion.div
      ...
      layout
      initial={{ opacity: 0 }}
      animate={{ opacity: 1, rotate: [-8, 5, -3, 2, 0] }}
      exit={{ opacity: 0 }}
      transition={{ type: 'spring', stiffness: 260, damping: 24 }}
      className={`card-face card-sheen relative rounded-2xl w-20 h-28 sm:w-24 sm:h-32 border-4 border-white/30 ${glowClass}`}
      style={{ background: `linear-gradient(150deg, ${gradFrom} 0%, ${gradTo} 100%)` }}
    >
      <div className="absolute inset-[10%] opacity-40" style={{ background: corEscuro, transform: 'rotate(-20deg)', borderRadius: '50% / 60%' }} />
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center p-3">
        ...
        {sub && <span className="mt-1 text-white/60 font-bold" style={{ fontSize: '8px', letterSpacing: '0.1em' }}>{sub}</span>}
      </div>
...
export default function Mesa() {
  ...
  // Flourish discreto quando uma carta de acao ou especial cai na pilha
  useEffect(() => {
    if (!topo) return;
    ...
    content = (
      <div className="uno-bg min-h-screen flex flex-col items-center justify-center gap-4">
        <div className="relative h-14 w-14">
          ...
          <div className="absolute inset-0 rounded-full border-4 border-transparent border-t-[oklch(0.63_0.24_27)] animate-spin" />
        </div>
        <p className="text-zinc-400 text-sm">Entrando na sala...</p>
      </div>
    );
  } else if (erro && !sala) {
    viewKey = 'erro';
    ...
    content = (
      <div className="uno-bg min-h-screen flex flex-col items-center justify-center px-4">
        <motion.div
          ...
          className="relative uno-card-surface rounded-3xl p-8 text-center max-w-sm w-full overflow-hidden"
        >
          <div className="pointer-events-none absolute inset-0 flex items-center justify-center overflow-hidden">
            {CONFETTI_PIECES.map((c) => (
              <motion.div
                ...
                transition={{ duration: 1.15, delay: 0.35 + c.delay, ease: 'easeOut' }}
          ...
          <motion.div
            ...
            className="relative mx-auto mb-3 flex h-16 w-16 items-center justify-center rounded-2xl bg-[oklch(0.86_0.17_85)]/15 ring-2 ring-[oklch(0.86_0.17_85)]/40"
          >
            <Trophy className="h-8 w-8 text-[oklch(0.86_0.17_85)]" />
          </motion.div>
          <p className="relative text-zinc-400 text-xs uppercase tracking-widest mb-2">Vencedor</p>
          <motion.div
          ...
          <motion.h2
            ...
            className="relative font-display text-5xl mb-6"
            translate="no"
          >
            {vencedorPlayer?.name}
          </motion.h2>
          <div className="relative flex gap-3">
            <button onClick={() => navigate('/')} className="flex-1 flex items-center justify-center gap-2 rounded-2xl bg-[oklch(0.22_0.035_265)]/60 py-3 text-sm text-zinc-300 ring-1 ring-white/10 hover:ring-white/25 transition">
              <Home className="h-4 w-4" /> Inicio
            </button>
            <button onClick={() => navigate('/jogo')} className="flex-1 flex items-center justify-center gap-2 rounded-2xl bg-[oklch(0.63_0.24_27)] py-3 text-sm font-bold text-white hover:bg-[oklch(0.68_0.24_27)] transition">
      ...
      <div className="uno-bg min-h-screen flex items-center justify-center px-4">
        ...
        >
          <div className="text-center mb-6">
            <p className="text-xs text-zinc-400 uppercase tracking-widest mb-2">Codigo da sala</p>
            <button
            ...
            >
              ...
              {copiado ? (
                <Check className="h-5 w-5 text-[oklch(0.68_0.2_152)]" />
              ) : (
                <Copy className="h-5 w-5 text-zinc-500 opacity-0 transition group-hover:opacity-100" />
              )}
            </button>
            <p className="text-xs text-zinc-500 mt-2">Toque no codigo para copiar e manda pros amigos!</p>
          </div>
              ...
              {sala?.jogadores?.map((j, i) => {
                ...
                return (
                  <motion.div
                    ...
                    className="flex items-center gap-3 rounded-2xl bg-[oklch(0.22_0.035_265)]/60 px-4 py-3 ring-1 ring-white/5"
                  >
          ...
          {isCriador ? (
            <button
              ...
              className="w-full rounded-2xl bg-[oklch(0.63_0.24_27)] py-3.5 text-sm font-bold uppercase tracking-wider text-white disabled:opacity-40 hover:bg-[oklch(0.68_0.24_27)] transition"
            >
            ...
            <p className="flex items-center justify-center gap-2 text-center text-sm text-zinc-400 py-2">
              <span className="h-1.5 w-1.5 rounded-full bg-[oklch(0.86_0.17_85)] animate-pulse" />
    ...
    content = (
      <div className="uno-bg relative min-h-screen flex flex-col overflow-hidden">
        <div
          ...
          style={{ background: corAtualObj ? `radial-gradient(ellipse at center, ${corAtualObj.hex}18 0%, transparent 70%)` : undefined }}
        />

        {/* Flourish de carta de acao/especial */}
        <AnimatePresence>
          {flourish && (
            <motion.div
              key={flourish.key}
              initial={{ opacity: 0, scale: 0.4 }}
              animate={{ opacity: [0, 0.6, 0], scale: [0.4, 1.7, 2] }}
              transition={{ duration: 0.85, ease: 'easeOut' }}
              className="pointer-events-none fixed inset-0 z-30 flex items-center justify-center"
            >
              <div
                className="h-44 w-44 rounded-full sm:h-72 sm:w-72"
                style={{ background: `radial-gradient(circle, ${COR[flourish.cor]?.hex || 'white'} 0%, transparent 70%)`, filter: 'blur(4px)' }}
              />
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence>
          {unoAnim && (
            ...
            >
              <div className="font-display text-[7rem] sm:text-[10rem] drop-shadow-2xl" style={{ color: 'oklch(0.86 0.17 85)', textShadow: '0 0 60px oklch(0.86 0.17 85 / 0.8)' }}>
                UNO!
              </div>
            </motion.div>
          )}
        ...
        <AnimatePresence>
          {modal === 'escolherCor' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-40 flex items-center justify-center bg-black/80 px-4 backdrop-blur-sm">
              <motion.div initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="uno-card-surface rounded-3xl p-6 w-full max-w-xs text-center">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {CORES_UNO.map((cor) => (
                    <button key={cor} onClick={() => enviarJogada(cor)} className={`${COR[cor].btn} rounded-2xl py-5 text-white font-bold text-sm transition shadow-lg`}>
                      {COR_LABEL[cor]}
                    </button>
                  ))}
                </div>
          ...
          {modal === 'acaoZero' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-40 flex items-center justify-center bg-black/80 px-4 backdrop-blur-sm">
              <motion.div initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="uno-card-surface rounded-3xl p-6 w-full max-w-xs text-center">
                ...
                <p className="text-sm text-zinc-400 mb-5">Escolha uma acao:</p>
                {!alvoZero ? (
                  <div className="flex gap-3">
                    <button onClick={() => setAlvoZero('ver')} className="flex-1 rounded-2xl bg-[oklch(0.6_0.22_255)] py-4 text-white font-bold text-sm hover:opacity-90 transition">
                      Ver cartas
                    </button>
                    <button onClick={() => setAlvoZero('trocar')} className="flex-1 rounded-2xl bg-[oklch(0.63_0.24_27)] py-4 text-white font-bold text-sm hover:opacity-90 transition">
                      Trocar mao
                    </button>
                  </div>
                ) : (
                  <div>
                    ...
                    <div className="space-y-2">
                      {estado.jogadores.filter((id) => id !== user.id).map((id) => {
                        const p = pById(id);
                        return (
                          <button key={id} onClick={() => confirmarAcaoZero(alvoZero, id)} className="w-full flex items-center gap-3 rounded-2xl bg-[oklch(0.22_0.035_265)]/60 px-4 py-3 hover:bg-[oklch(0.28_0.04_265)] transition ring-1 ring-white/10">
        ...
        <AnimatePresence>
          {chatAberto && (
            ...
            >
              ...
              <button onClick={() => setChatAberto(false)} className="mt-2 w-full text-xs text-zinc-500 hover:text-zinc-300 transition">
                Fechar
              </button>
            </motion.div>
          )}
        ...
        <div className="relative z-10 flex flex-col min-h-screen p-2 sm:p-3 gap-2">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {estado.jogadores.filter((id) => id !== user.id).map((id) => {
              ...
              return (
                <motion.div
                  key={id}
                  animate={ehVez ? { scale: [1, 1.015, 1] } : { scale: 1 }}
                  transition={{ repeat: ehVez ? Infinity : 0, duration: 1.6 }}
                  className={`uno-card-surface rounded-2xl p-3 flex flex-col gap-2 transition-all ${ehVez ? 'turn-glow ring-1 ring-white/10' : 'ring-1 ring-white/5'}`}
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {ehVez && <p className="text-[10px] font-bold text-[oklch(0.86_0.17_85)]">vez dele</p>}
                    </div>
                    {temUno && <span className="text-[10px] font-black text-[oklch(0.86_0.17_85)] bg-[oklch(0.86_0.17_85)]/10 px-1.5 py-0.5 rounded-full">UNO</span>}
                  </div>
          ...
          {/* Mesa central - superficie com profundidade */}
          <div className="relative flex-1 flex flex-col items-center justify-center gap-3 py-2">
            <div className="game-table pointer-events-none absolute left-1/2 top-1/2 h-[78%] w-[94%] -translate-x-1/2 -translate-y-1/2 rounded-[50%] sm:w-[85%]" />

            {estado.corAtual && (
              <div className={`relative flex items-center gap-2 px-3 py-1.5 rounded-full ${COR[estado.corAtual]?.bg || ''} shadow-lg`}>
            ...
            <div className="relative flex items-center gap-5 sm:gap-8">
              ...
              <div className="flex flex-col items-center gap-1">
                <motion.button
                  ...
                  className={`relative flex items-center justify-center rounded-2xl border-2 border-white/20 w-16 sm:w-20 transition-all ${ehMinhVez ? 'cursor-pointer hover:border-white/60 bg-gradient-to-br from-[oklch(0.25_0.05_265)] to-[oklch(0.15_0.03_265)]' : 'cursor-not-allowed opacity-50 bg-gradient-to-br from-[oklch(0.2_0.04_265)] to-[oklch(0.12_0.02_265)]'}`}
                  style={{ height: '88px' }}
                >
                  ...
                  {estado.acumulado > 0 && (
                    <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute -top-2.5 -right-2.5 bg-[oklch(0.63_0.24_27)] text-white text-xs font-black rounded-full w-7 h-7 flex items-center justify-center shadow-lg ring-2 ring-[oklch(0.14_0.03_265)]">
                      +{estado.acumulado}
                    </motion.span>
                  )}
                </motion.button>
            ...
            {/* Indicador de turno elegante */}
            <div className="relative text-center">
              {ehMinhVez ? (
                <motion.div
                  initial={{ scale: 0.9, opacity: 0 }}
                  animate={{ scale: 1, opacity: 1 }}
                  className="inline-flex items-center gap-2 rounded-full bg-[oklch(0.86_0.17_85)]/12 px-4 py-1.5 ring-1 ring-[oklch(0.86_0.17_85)]/40"
                >
                  <span className="relative flex h-2 w-2">
                    <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[oklch(0.86_0.17_85)] opacity-75" />
                    <span className="relative inline-flex h-2 w-2 rounded-full bg-[oklch(0.86_0.17_85)]" />
                  </span>
                  <span className="text-sm font-bold text-[oklch(0.86_0.17_85)]">Sua vez!</span>
                </motion.div>
              ) : (
              ...
              {estado.acumulado > 0 && (
                <motion.p initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="text-xs font-bold text-[oklch(0.85_0.18_27)] mt-1 bg-[oklch(0.63_0.24_27)]/15 px-3 py-1 rounded-full inline-block">
            ...
            {ehMinhVez && estado.fase === 'acaoZero' && (
              <motion.button initial={{ scale: 0 }} animate={{ scale: 1 }} onClick={() => setModal('acaoZero')} className="relative rounded-2xl bg-[oklch(0.85_0.18_90)] px-5 py-2.5 text-sm font-bold text-zinc-900 shadow-lg hover:opacity-90 transition">
          ...
          <div className="uno-card-surface rounded-3xl p-3 sm:p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex gap-2">
                {minhaMao.length === 1 && (
                  <motion.button initial={{ scale: 0 }} animate={{ scale: 1 }} whileTap={{ scale: 0.9 }} onClick={declararUno} className="rounded-xl bg-[oklch(0.86_0.17_85)] px-4 py-1.5 text-xs font-black text-zinc-900 uppercase tracking-wider shadow-[0_4px_15px_oklch(0.86_0.17_85/0.5)] hover:opacity-90 transition">
                    UNO!
                  </motion.button>
                )}
                {cartasSelecionadas.length > 0 && ehMinhVez && (
                  <motion.button initial={{ scale: 0 }} animate={{ scale: 1 }} whileTap={{ scale: 0.95 }} onClick={jogar} className="rounded-xl bg-[oklch(0.63_0.24_27)] px-4 py-1.5 text-xs font-bold text-white uppercase tracking-wider hover:bg-[oklch(0.68_0.24_27)] transition shadow-lg">
            ...
            {erro && (
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-2 text-xs text-[oklch(0.85_0.18_27)] text-center bg-[oklch(0.63_0.24_27)]/10 px-3 py-1.5 rounded-xl">
                {erro}
              </motion.p>
            )}
          </div>
